...
bool testRayOBB(glm::vec3 o, glm::vec3 t, AbstractModel::OBB collider){
	glm::quat qinv= glm::inverse(collider.u); //Quaternion inverso
	glm::vec3 oInv = qinv * o; //Transformar el origen, destino y centro de la caja cn el inverso
	glm::vec3 tInv = qinv * t;
	glm::vec3 cOBBInv = qinv * collider.c;
	AbstractModel::AABB colliderABB;
	colliderABB.maxs = cOBBInv + collider.e; //Crear un collider ABB
	colliderABB.mins = cOBBInv - collider.e;
	return intersectSegmentAABB(oInv, tInv, colliderABB); //Utilizar el metodo de collision de un rayo con una caja alineada
